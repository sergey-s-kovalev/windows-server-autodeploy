...
# Создаем группу имеющую право управлять другими рабочими столами
New-LocalGroup -Name AllowRDSShadow -Description "Users who can connect to others"

#Add-LocalGroupMember -Group "Remote Desktop Users" -Member AIvanov, APetrov
#Add-LocalGroupMember -Group AllowRDSShadow -Member AIvanov

# Ставим терминальные службы
...
# Даем право локальной группе управлять 
$Group = "AllowRDSShadow"
(Get-WmiObject -Class "Win32_TSPermissionsSetting" -Namespace "root\CIMV2\terminalservices" -ComputerName $env:COMPUTERNAME -Authentication PacketPrivacy -Impersonation Impersonate).AddAccount($Group, 2)


# Указываем тип управления
